...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "\n" + _("SSL options: (see the PikachuCoin Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single PikachuCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PikachuCoin is probably already running."), strDataDir.c_str()));
    ...
    // requested to kill PikachuCoin-qt during the last operation. If so, exit.
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Wallet needed to be rewritten: restart PikachuCoin to complete") << "\n";
