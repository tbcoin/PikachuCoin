...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of PikachuCoin before 0.8.6 had a bug that inserted
...
#ifdef BITCITY_SHARES
int64 CWallet::GetBalanceOfMySingleAddress( const CTxDestination & dest )
{
	map<CTxDestination, int64> balances = GetAddressBalances();
	map<CTxDestination, int64>::iterator it = balances.find( dest );
	return it == balances.end() ? 0 : it->second;
}

void CWallet::ScanForAllMoneyAddress( std::map< CTxDestination, int64 > & mapAddressWithMoney, int nLimitHeight )
{
	mapAddressWithMoney.clear();

    CBlockIndex* pindex = pindexGenesisBlock;
    {
        LOCK(cs_wallet);
        while ( pindex && pindex->nHeight <= nLimitHeight )
        {
            CBlock block;
            block.ReadFromDisk(pindex);
            BOOST_FOREACH(CTransaction& tx, block.vtx)
            {
				...
				if (nDepth < (wtx.IsFromMe() ? 0 : 1))
					continue;

				uint256 hashvalue = wtx.GetHash();

				LOCK(mempool.cs);
				CCoins coins;
				CCoinsViewMemPool view(*pcoinsTip, mempool);
				if (view.GetCoins(hashvalue, coins))
				{
					for (unsigned int i = 0; i < wtx.vout.size(); i++) {
						if ( wtx.vout[i].nValue > 0 )
						{				
							if ( coins.IsAvailable(i) )
							{
								...
								{
									if ( mapAddressWithMoney.find(address) ==  mapAddressWithMoney.end())
									{
										mapAddressWithMoney[address] = tx.vout[i].nValue;
									}
									else
									{
										mapAddressWithMoney[address] += tx.vout[i].nValue;
				...
				if ( !wtx.IsCoinBase() )
				{
					for (unsigned int j = 0; j < wtx.vin.size(); j++ )
					{
						const COutPoint &prevout = wtx.vin[j].prevout;
						CCoins coins;
						...
						{
							mempool.pruneSpent(prevout.hash, coins); // TODO: this should be done by the CCoinsViewMemPool
							if ( coins.vout[prevout.n].nValue > (int64)0 )
							{
								CTxDestination address;
								if (ExtractDestination(coins.vout[prevout.n].scriptPubKey, address))
								{
									if ( mapAddressWithMoney.find(address) ==  mapAddressWithMoney.end())
									{
										mapAddressWithMoney[address] = -coins.vout[prevout.n].nValue;
									}
									else
									{
										mapAddressWithMoney[address] -= coins.vout[prevout.n].nValue;
...
void CWallet::GetBindedAddress( std::map< std::string, std::string > & mapBindedAddress )
{
	mapBindedAddress.clear();
    ...
    for (CWallet::TxItems::iterator it = txOrdered.begin(); it != txOrdered.end(); ++it)
    {
        ...
        if (pwtx != 0)
		{
			if ( !pwtx->strUserArgs.empty() )
			{
				json_spirit::Value value;
				if (!read_string(pwtx->strUserArgs, value))
				{}
				else
				{
					const json_spirit::Object& request = value.get_obj();
					std::string bindedbtcstr = json_spirit::find_value(request, "bindedbtc").get_str();
					std::vector<std::string> addresses = EEE::utility::split(bindedbtcstr, ":");
					if (addresses.size() == 2)
					{
						mapBindedAddress[addresses[0]] = addresses[1];
					}
				}
			}
		}
